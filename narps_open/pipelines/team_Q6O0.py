...
class PipelineTeamQ60O(Pipeline):
    ...
    def get_subject_infos_gain(event_files, runs):
            ...
            with open(event_file, 'rt') as file:
                ...
                for line in file:
                    ...
                    for condition in condition_names:
                        val = cond + '_run' + str(run_id + 1) # trial_run1
        ...
        for r in range(len(runs)):
            ...
            cond = [c + '_run' + str(run_id + 1) for c in condition_names]
    ...
    def get_subject_infos_loss(event_files):
        ...
        for r in range(len(runs)):  # Loop over number of runs.
            onset.update({s + '_run' + str(r+1) : [] for s in condition_names}) # creates dictionary items with empty lists
            duration.update({s + '_run' + str(r+1) : [] for s in condition_names}) 
        ...
        for r in range(len(runs)):
            ...
            cond = [c + '_run' + str(r+1) for c in condition_names]
    ...
    def get_parameters_file(filepaths, subject_id, result_dir, working_dir):
        ...
        for i, file in enumerate(filepaths):
            ...
            if not os.path.isdir(join(result_dir, working_dir, 'parameters_file')):
                os.mkdir(join(result_dir, working_dir, 'parameters_file'))
    ...
    def rm_gunzip_files(files, subject_id, result_dir, working_dir):
        ...
        gunzip_dir = join(result_dir, working_dir, 'l1_analysis', f"_subject_id_{subject_id}", 'gunzip_func')
    ...
    def rm_smoothed_files(files, subject_id, result_dir, working_dir):
        ...
        smooth_dir = join(result_dir, working_dir, 'l1_analysis', f"_subject_id_{subject_id}", 'smooth')
    ...
    def get_l1_analysis(subject_list, TR, fwhm, run_list, exp_dir, result_dir, working_dir, output_dir):
        ...
        func_file = join('derivatives', 'fmriprep', 'sub-{subject_id}', 'func', 
                        'sub-{subject_id}_task-MGT_run-*_bold_space-MNI152NLin2009cAsym_preproc.nii.gz')

        event_file = join('sub-{subject_id}', 'func', 
                        'sub-{subject_id}_task-MGT_run-*_events.tsv')

        param_file = join('derivatives', 'fmriprep', 'sub-{subject_id}', 'func', 
        ...
        l1_analysis = Workflow(base_dir = join(result_dir, working_dir), name = "l1_analysis")
    ...
    def get_l2_analysis(subject_list, n_sub, model_list, method, exp_dir, result_dir, working_dir, output_dir):
        ...
        # SelectFiles
        contrast_file = join(result_dir, output_dir, "l1_analysis_{model_type}", "_subject_id_*", "con_0001.nii")
        ...
        l2_analysis = Workflow(base_dir = join(result_dir, working_dir), name = 'l2_analysis')
    ...
    def reorganize_results(result_dir, output_dir, n_sub, team_ID):
        ...
        h1 = join(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_model_type_gain')
        h2 = join(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_model_type_gain')
        h3 = join(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_model_type_gain')
        h4 = join(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_model_type_gain')
        h5 = join(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_model_type_loss')
        h6 = join(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_model_type_loss')
        h7 = join(result_dir, output_dir, f"l2_analysis_equalIndifference_nsub_{n_sub}", '_model_type_loss')
        h8 = join(result_dir, output_dir, f"l2_analysis_equalRange_nsub_{n_sub}", '_model_type_loss')
        h9 = join(result_dir, output_dir, f"l2_analysis_groupComp_nsub_{n_sub}", '_model_type_loss')

        h = [h1, h2, h3, h4, h5, h6, h7, h8, h9]

        repro_unthresh = [join(filename, "spmT_0002.nii") if i in [4, 5] else join(filename, "spmT_0001.nii") for i, filename in enumerate(h)]

        repro_thresh = [join(filename, "_threshold1", 
            "spmT_0002_thr.nii") if i in [4, 5] else join(filename, "_threshold0", "spmT_0001_thr.nii")  for i, filename in enumerate(h)]
